...
public class UserInfoSignFriendR2dbcDao implements UserInfoSignFriendDao {
    ...
    private Publisher<? extends Result> executeReadById(long friendId, long userId, Connection connection) {
        return connection.createStatement(READ_BY_ID)
                .bind(0, userId)
                .bind(1, friendId)
                .bind(2, friendId)
                .execute();
    ...
    private Publisher<? extends Result> executeReadAllUsers(long id, Connection connection) {
        return connection.createStatement(READ_ALL_USERS)
                .bind(0, id)
                .bind(1, id)
                .execute();
    }
